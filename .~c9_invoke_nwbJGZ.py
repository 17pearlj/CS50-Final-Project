...
def save():
    output = db.execute("SELECT * FROM active WHERE userid = :userid", userid=session["user_id"])
    if request.method == "POST":
        db.execute("INSERT INTO favs (userid, tonic, style, timeSignature, tonality, energy, tempo, measures) VALUES (userid, :name, tonic,:style, :timeSignature, :tonality, :energy, :tempo, :measures)")
